...
def rank_pro(lb,md, expr_data):

    global sample0,sample1
    index_0 = []
    index_1 = []
...
def ES_for_permutation(lb, md, hit_matrix_filtered, weighted_score_type, expr_data, gene_num):
    global sample0,sample1
    index_0 = []
    index_1 = []
    for i,j in enumerate(lb):   #abstract index as i, content as j
...
def ES_null(lb, times, method, hit_matrix_filtered, weighted_score_type,expr_data,gene_num):
    lb_matrix = np.array([lb for i in range(times)])
    ran_labels = np.array(map(lambda x: np.random.permutation(x), lb_matrix))
    def_get_es_null = lambda x: ES_for_permutation(x,method ,hit_matrix_filtered,weighted_score_type,expr_data,gene_num)
