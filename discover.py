...
class TwoSampleTest(OneSampleTest):
    ...
    def __call__(self, result = None):
        ...
        if isinstance(sample2, Distr):
            for term in sample1.keys():
                if not sample2.has_key(term):
                    sample1.pop(term)
            n1, n2 = sample1.size(), sample2.size()
            for term, queries in sample1.items():
                m1, m2 = len(queries), len(sample2[term])
                result.append(list(term) + ['%d' % m1, '%d' % m2, self.calc_pvalue(m1, n1, m2, n2), '|'.join(queries)])
        else:
            for term in sample1.keys():
                if not term in sample2.keys():
                    sample1.pop(term)
            n1, n2 = sample1.size(), sum(sample2.values())
            for term, queries in sample1.items():
                m1, m2 = len(queries), sample2[term]
                try:
                    result.append(list(term) + ['%d' % m1, '%d' % m2, self.calc_pvalue(m1, n1, m2, n2), '|'.join(queries)])
                except:
                    #print term, m1, m2
